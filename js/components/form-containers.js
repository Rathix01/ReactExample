...
var containerBaseMixin = {
  ...
  componentDidMount: function() {
    TweenMax.fromTo( this, 0.4, { state: { opacity:0 } }, { state: { opacity: 1 },  ease: Power1.easeOut } );
  },
...
Components.register( "Group", React.createClass({
    ...
    getChild: function( item ) {
      return Components.createInstance( item.type, ramda.mixin( item, this.getColumns() ) )
    },

    setItem: function( item ) {
      return this.props.columns > 0
        ? Components.createInstance( item.type, ramda.mixin( item, this.getColumns() ) )
